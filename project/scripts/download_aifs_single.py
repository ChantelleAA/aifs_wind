...
aifs_config = config["MODELS"]["aifs-single"]
...
def target_path(date_str: str, init_time: str, typ: str, step_h: int,
    ...
    full_path = (Path(out_dir) / "aifs-single" / f"{stream}" / f"date{date_str}" /
...
def main():
    logger.info("="*80)
    logger.info("Starting AIFS-Single download")
    ...
    logger.info("="*80)
    logger.info("AIFS-Single download complete")
    logger.info("="*80)
